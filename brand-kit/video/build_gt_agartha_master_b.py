"""
GT-AGARTHA-MASTER (Variant B) — A/B test pair to gt_agartha_master.py
─────────────────────────────────────────────────────────
IDENTICAL to the A variant in every section EXCEPT the CTA.

  A (control):   "DM TO KNOW / THE PRICES NOW."       — high-intent, price-driven
  B (this one):  "COMMENT 'GREEN' / WE DM THE AQI DOC." — low-intent, lead capture

Hypothesis: a softer information-led CTA may pull more cold-traffic DMs from
real-estate-curious viewers who aren't yet price-shopping. The hard-sell CTA
qualifies harder but may filter out the top of the funnel.

Test design (Meta A/B at ₹500/day × 7 days):
  - Same audience (Hyderabad, real estate intent, age 28-55)
  - Same placement (Reels + Feed)
  - Same objective (OUTCOME_ENGAGEMENT, optimized for POST_ENGAGEMENT)
  - Only this CTA text differs — single-variable test
"""
...
OUT_ROOT = REPO / "out"
FRAMES_DIR = OUT_ROOT / "frames_gt-agartha-master-b"
SILENT_MP4 = OUT_ROOT / "brandmint-gt-agartha-master-b-silent.mp4"
...
def text_for_section(section_name: str, t_local: float, dur: float) -> str:
    ...
    if section_name == "HOOK":
        ...
        # VARIANT B — soft-sell, information-led CTA (A/B test against the
        # 'DM TO KNOW THE PRICES NOW' control in build_gt_agartha_master.py).
        parts.append(serif_pullquote("the AQI is a public record.",
        ...
        parts.append(svg_text("AQI + NOISE DOC.", CX, 1075, 66,
